package com.alibaba.datax.plugin.reader.modifiedhdfsreader;
...
public class DFSUtil {
    ...
    public void parquetFileStartRead(String sourceParquetFile, Configuration readerSliceConfig,
        ...
        // 判断是否读取所有列
        if (null == column || column.size() == 0) {
            int allColumnsCount = getParquetAllColumnsCount(sourceParquetFile);
            columnIndexMax = allColumnsCount - 1;
            isReadAllColumns = true;
        } else {
            columnIndexMax = getMaxIndex(column);
        ...
        if (columnIndexMax >= 0) {
            JobConf conf = new JobConf(hadoopConf);
            Path parquetFilePath = new Path(sourceParquetFile);
            Properties p = new Properties();
            p.setProperty("columns", allColumns.toString());
            p.setProperty("columns.types", allColumnTypes.toString());
            try {
    ...
    private Integer getParquetAllColumnsCount(String filePath) {
        ParquetReader.Builder builder = ParquetReader.builder(new GroupReadSupport(), new Path(filePath));
        try (ParquetReader<Group> reader = builder.build()) {
            return reader.read().getType().getFieldCount();
        } catch (IOException e) {
            return null;
        }
    }
    ...
    public boolean checkHdfsFileType(String filepath, String specifiedFileType) {
        ...
        try {
            ...
            if (StringUtils.equalsIgnoreCase(specifiedFileType, Constant.CSV)
                ...
                // TODO: 6/3/20 实现判断是否是 Parquet File
